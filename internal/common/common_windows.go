...
func ConvertDOSPath(p string) string {
	...
	// Extrai o prefixo de dispositivo (ex: \\Device\\HarddiskVolume1)
	parts := strings.SplitN(p, `\`, 4)
	if len(parts) < 3 {
		return p
	}
	rawDrive := `\` + parts[1] + `\` + parts[2]

	szTarget := make([]uint16, 512)

	for d := 'A'; d <= 'Z'; d++ {
		szDeviceName := string(d) + ":"
		deviceNamePtr, err := windows.UTF16PtrFromString(szDeviceName)
		if err != nil {
			continue
		}
		...
		targetPath := windows.UTF16ToString(szTarget)
		if strings.EqualFold(targetPath, rawDrive) {
			suffix := ""
			if len(parts) > 3 {
				suffix = `\` + parts[3]
			}
			return filepath.Join(szDeviceName, suffix)
		}
	}
		...
		if st == STATUS_BUFFER_OVERFLOW || st == STATUS_BUFFER_TOO_SMALL || st == STATUS_INFO_LENGTH_MISMATCH {
			if int(*resultLength) <= cap(*buf) {
				*buf = (*buf)[:int(*resultLength)]
				...
				// dobra o tamanho ou aloca exatamente resultLength, conforme necessidade
				newSize := int(*resultLength)
